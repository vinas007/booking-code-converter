...
export class StakeHttpClient {
  ...
  async get<T>(path: string): Promise<T> {
    if (!this.hasApiKey()) {
      throw new StakeApiErrorClass(
        "missing_api_key",
        "Stake API key is not configured. Set STAKE_ODDS_API_KEY to use Stake Sports Data API.",
      );
    }
    ...
    try {
      ...
      throw new StakeApiErrorClass(
        "network_error",
        `Network error contacting Stake API: ${err instanceof Error ? err.message : String(err)}`,
      );
    } finally {
  ...
  async getSports(): Promise<unknown> {
    return this.get("/api/sports");
  }

  async getSportCategories(sportSlug: string): Promise<unknown> {
    return this.get(`/api/sports/${sportSlug}/categories`);
  }

  async getCategoryTournaments(sportSlug: string, categorySlug: string): Promise<unknown> {
    return this.get(`/api/sports/${sportSlug}/categories/${categorySlug}/tournaments`);
  }

  async getTournamentFixtures(sportSlug: string, categorySlug: string, tournamentSlug: string): Promise<unknown> {
    return this.get(`/api/sports/${sportSlug}/categories/${categorySlug}/tournaments/${tournamentSlug}/fixtures`);
  }

  async getFixtureOdds(fixtureSlug: string): Promise<unknown> {
